...
def read_excel(excelstr,sheet_name,case_name,search_field):
    workbook = xlrd.open_workbook(excelstr)
    sheet= workbook.sheet_by_name(sheet_name)
    for i in range(sheet.nrows):
        if sheet.col_values(0)[i]==case_name:
            if sheet.col_values(1)[i]==search_field:
                return sheet.col_values(2)[i]            
